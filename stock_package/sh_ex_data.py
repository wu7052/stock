...
class sh_web_data(ex_web_data):
    ...
    def basic_info_json_parse(self, json_str=None):
        ...
        if json_str is not None:
            ...
            stock_matix = [company_code, company_abbr, total_shares, total_flow_shares, list_date, full_name]
            df = pd.DataFrame(stock_matix)
            df1 = df.T
            df1.rename(columns={0: 'ID', 1: 'Name', 2: 'Total Shares', 3: 'Flow Shares', 4: 'List_Date', 5: 'Full_Name'}, inplace=True)
            # df1.sort_values(by=['Total Shares'], inplace=True)
            # print(df1.describe())
            # print(df1)
            return df1
