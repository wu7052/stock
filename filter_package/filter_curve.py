...
class filter_curve:
    ...
    def __init__(self, f_conf='', f_start_date='',  f_end_date='', data_src='qfq'):
        ...
        try:
            ...
            self.filter_growth_below_pct = self.f_conf.rd_opt('filter_request', 'filter_growth_below_pct')
            self.filter_high_left_power_request = self.f_conf.rd_opt('filter_request', 'filter_high_left_power_request')
            self.filter_high_right_power_request = self.f_conf.rd_opt('filter_request', 'filter_high_right_power_request')
            self.filter_cur_left_power_request = self.f_conf.rd_opt('filter_request', 'filter_cur_left_power_request')
            self.filter_golden_pct = self.f_conf.rd_opt('filter_request', 'filter_golden_pct')
            self.filter_golden_pct_request = float(self.f_conf.rd_opt('filter_request', 'filter_golden_pct_request'))
            ...
            wx.info("[Filter_Curve] : __init__ called")

            # data_src == 'bt_qfq', 则必须指定日期区间，用于回测
            # data_src == 'cq' 或 'qfq', 日期区间是 从过去的240个自然日 ~ 当前交易日
            if data_src == 'cq' or data_src == 'qfq':
                ...
                if f_end_date=='' or f_start_date=='':
                    wx.info("[Filter_002]回测未设置起止日期区间[{}-{}]".format(f_start_date, f_end_date))
                    return
    ...
    """
    函数说明：处理 单个股票 左侧 或 右侧的数据，根据规则过滤后，返回 数组[high_left_power, high_right_power, cur_left_power]
    high_left_power： 高点左侧的得分
    high_right_power：高点右侧的得分
    cur_left_power：最近交易日左侧的得分
    """
    ...
    def filter_side(self):
        ...
        for t_name in tname_arr:
            ...
            wx.info("[Filter_Curve] Completed Located the Highest Price Point in {}".format(t_name))
            ...
            for count, df_each_stock in enumerate(df_groupby_id):
                ...
                if ret_LR == 1:
                    wx.info("[Filter_Curve LR] {} / {} Filter {} LR ... [LR FOUND!!!] ".format(count + 1, len(df_groupby_id),
                                                                                          df_each_stock[0]))
                elif ret_LR == 0:  # 左侧涨幅不达标
                    wx.info("[Filter_Curve LR] {} / {} Filter {} Left Raise Low... [LR PASS] ".format(count + 1,  len(df_groupby_id),
                                                                                                 df_each_stock[0]))
                    continue
                elif ret_LR == -1: # 右侧创新低
                    wx.info("[Filter_Curve LR] {} / {} Filter {} Right New Lowest ... [LR PASS] ".format(count + 1, len(df_groupby_id),
                                                                                      df_each_stock[0]))
                    continue
                ...
                if min_golden_pct > self.filter_golden_pct_request:
                    wx.info("[Filter_Curve Golden] {} / {} Filter {}  ........ [Golden PCT FOUND!!!] ".format(count + 1,
                                                                                                len(df_groupby_id),
                                                                                                df_each_stock[0]))
                else:
                    wx.info("[Filter_Curve Golden] {} / {} Filter {}  ........ [Golden PCT PASS] ".format(count + 1,
                                                                                                len(df_groupby_id),
                                                                                                df_each_stock[0]))
                    continue
                ...
                if arr_power[0] < int(self.filter_high_left_power_request):
                    wx.info("[Filter_Curve Power] {} / {} Filter {} High Left Side Power... [PASS] ".format(count + 1,
                                                                                                len(df_groupby_id),
                                                                                                df_each_stock[0]))
                    continue
                elif arr_power[1] < int(self.filter_high_right_power_request):
                    wx.info("[Filter_Curve Power] {} / {} Filter {} High Right Side Power... [PASS] ".format(count + 1,
                                                                                                len(df_groupby_id),
                                                                                                df_each_stock[0]))
                    continue
                elif arr_power[2] < int(self.filter_cur_left_power_request):
                    wx.info(
                        "[Filter_Curve Power] {} / {} Filter {} Cur Left Side Power... [PASS] ".format(count + 1,
                                                                                            len(df_groupby_id),
                                                                                            df_each_stock[0]))
                    continue
                else:
                    wx.info("[Filter_Curve Power] {} / {} Filter {}  Power........ [FOUND!] ".format(count + 1,
                                                                                                len(df_groupby_id),
                                                                                                df_each_stock[0]))
